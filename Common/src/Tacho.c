...
#include "FRTOS1.h"
  ...
  /*! \todo Set appropriate values for NOF_ROUND_PULSE, SPEED_CALC_PERIOD_MS and NOF_HISTORY */
...
void TACHO_CalcSpeed(void) {
  ...
  speedLeft = (int32_t)(deltaLeft * 1000/(SPEED_CALC_PERIOD_MS*(NOF_HISTORY-1)));
  if (negLeft) {
    speedLeft = -speedLeft;
  }
  speedRight = (int32_t)(deltaRight * 1000/(SPEED_CALC_PERIOD_MS*(NOF_HISTORY-1)));
  if (negRight) {
...
void TACHO_Sample(void) {
  ...
  if (cnt < SPEED_CALC_PERIOD_MS) { // Sampling alle 5 ms
    return;
  }
  cnt = 0; /* reset counter */
  /* left */
